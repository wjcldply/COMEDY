...
assert (
    len(raw) == 3
    and len(raw["session2"]) == len(raw["session3"]) == len(raw["session4"]) == 1000
)
...
for session_N in session_Ns:
    ...
    session_N_list = (
        []
    )  # list of dict (each dict: 'history_sessions', 'current_session_original', 'current_session_test'(cutoff randomly), 'id')
    for sample in raw[
        session_N
    ]:  # sample = {'history_sessions', 'current_session', 'id}
        modified_sample = {}
        ...
        rand_cutoff_idx = random.choice(
            [
                i
                for i in range(
                    int(len(sample["current_session"]) * 0.3),
                    int(len(sample["current_session"]) * 0.7) + 1,
                )
                if i % 2 == 0
            ]
        )
        modified_sample["current_session_test"] = sample["current_session"][
            : rand_cutoff_idx + 1
        ]
