...
def parse_args():
    ...
    parser.add_argument('--add_sft',
                        action='store_true',
                        help='add_sft loss for training')
    ## Tokenizer
    parser.add_argument(
        "--add_eot_token",
        action='store_true',
...
def main():
    ...
    #     args.output_dir, train_phase, args.seed, tokenizer,
    #     args.max_seq_len)
    train_dataset = get_reward_data(args, tokenizer, args.data_path)
    eval_dataset = get_reward_data(args, tokenizer, args.data_path)
    # print(train_dataset)
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            # exit()
            if args.add_sft:
                sft_loss = -get_batch_logps(outputs.logits[:batch_size], batch['input_ids'][:batch_size], label_mask[:batch_size]).mean()
            ...
            # total_steps
            mean_loss += loss.item()
            if args.print_loss and global_steps > 0 and global_steps % args.log_interval == 0:
                try:    
                    print(
                ...
                if args.global_rank == 0:
                    writer.add_scalar("Train/loss", mean_loss/(step+1),  global_steps)
            model.backward(loss)
            model.step()
            end = time.time()
        ...
        chosen_rewards, rejected_rewards, eval_loss = evaluation(model, ref_model, tokenizer, eval_dataloader)
        if args.global_rank == 0:
            writer.add_scalar("Dev/chosen_rewards", chosen_rewards,  global_steps)
            writer.add_scalar("Dev/rejected_rewards", rejected_rewards, global_steps)
            writer.add_scalar("Dev/rejected_rewards", eval_loss, global_steps)
        print_rank_0(f"chosen: {chosen_rewards}, rejected: {rejected_rewards}, loss: {eval_loss}", args.global_rank)
