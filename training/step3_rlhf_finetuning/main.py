...
def parse_args():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="unsupervised training enabled",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--actor_weight_decay", type=float, default=0.1, help="Weight decay to use."
    )
    parser.add_argument(
        "--critic_weight_decay", type=float, default=0.1, help="Weight decay to use."
    )
    parser.add_argument(
        "--num_train_epochs",
        type=int,
    ...
    parser.add_argument(
        "--max_seq_len", type=int, default=0
    )  # stage3 不要用这个，用prompt+ans那个长度！！！
...
def create_datasets(args, tokenizer, train_phase=3):
    unsupervised_training_enabled = args.unsupervised_training_enabled

    prompt_train_dataset = get_prompt_data(args, tokenizer)
    if unsupervised_training_enabled:
        unsupervised_train_dataset = get_unsupervised_data(
            args, tokenizer, args.unsupervised_dataset_name, train_phase
        )
    else:
...
def main():
    ...
    # xllm3.0的eos是[EOS], pad是[PAD]
    # xllm2.0的eos是[END], pad是[PAD]
    # llama的bos是1，eos是2，pad是0
    tokenizer = LlamaTokenizer.from_pretrained(
        args.actor_model_name_or_path, fast_tokenizer=True
    )
    ...
    print_rank_0("****************************")
    print_rank_0(f"number of total iters is {num_total_iters}")
    print_rank_0(f"len of prompt_train_data_loader is {len(prompt_train_dataloader)}")
    print_rank_0(
        f"len of unsupervised_train_data_loader is {len(unsupervised_train_dataloader)}"
    )
    ...
    args.end_of_conversation_token = ""  # 这个不要紧，用不到
            ...
            if exp_dataset is not None:
                ...
                for ppo_ep in range(args.ppo_epochs):
                    for i, (exp_data, unsup_data) in enumerate(
                        ...
                        # perform opt step
                        # trainer.perform_actor_critic_model_opt_step()
                ...
                if args.global_rank == 0:
                    writer.add_scalar(
                        "Train/actor_loss", actor_loss_sum / inner_iter, global_step
                    )
                    writer.add_scalar(
                        "Train/critic_loss", critic_loss_sum / inner_iter, global_step
                    )
                    writer.add_scalar(
                        "Train/unsuper_loss", unsup_loss_sum / inner_iter, global_step
                    )
                print_rank_0(
                ...
                if torch.distributed.get_rank() == 0:
                    writer.add_scalar(
                        "Train/reward_score", average_reward / inner_iter, global_step
                    )
                print_rank_0(
                ...
                if global_step % args.save_interval == 0:
                    ...
                    if args.actor_zero_stage == 3:
                        save_zero_three_model(
                            ...
                            save_dir=os.path.join(
                                args.output_dir, "actor" + "/" + str(global_step)
                            ),
                            zero_stage=args.actor_zero_stage,
                        )
                        if args.enable_ema:
                        ...
                        if args.global_rank == 0:
                            save_hf_format(
                                ...
                                tokenizer,
                                args,
                                sub_folder="actor" + "/" + str(global_step),
                            )
                            ...
                            if args.enable_ema:
                                save_hf_format(
                                    ...
                                    tokenizer,
                                    args,
                                    sub_folder="actor_ema" + "/" + str(global_step),
                                )
                    ...
                    if args.critic_zero_stage == 3:
                        save_zero_three_model(
                            ...
                            global_rank=args.global_rank,
                            save_dir=os.path.join(
                                args.output_dir, "critic" + "/" + str(global_step)
                            ),
                        ...
                        if args.global_rank == 0:
                            save_hf_format(
                                ...
                                tokenizer,
                                args,
                                sub_folder="critic" + "/" + str(global_step),
                            )
