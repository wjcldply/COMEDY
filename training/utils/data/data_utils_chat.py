...
def get_raw_dataset(dataset_name, output_path, seed, local_rank, train_data_path=""):
    if dataset_name == "Dahoas/rm-static":
        ...
        return raw_datasets.XiaoiceLabelDataset(
            output_path, seed, local_rank, train_data_path
        )
    else:
        raise RuntimeError(
    ...
    if train_phase == 1:
        ...
        for i, tmp_data in enumerate(current_dataset):
            ...
            if prompt is not None:
                ...
                prompt_token["attention_mask"] = [0] * add_count + prompt_token[
                    "attention_mask"
                ]
                prompt_token = {
                    "input_ids": torch.tensor(
                        prompt_token["input_ids"], dtype=torch.long
                    ),
                    "attention_mask": torch.tensor(
                        prompt_token["attention_mask"], dtype=torch.long
                    ),
                }
...
def create_dataset(
    ...
    raw_dataset = get_raw_dataset(
        dataset_name, output_path, seed, local_rank, train_data_path
    )
...
def get_prompt_data(args, tokenizer):
    ...
    prompt_raw_datasets = load_dataset(
        "json", data_files=data_files, split="train", name="prompt_dataset"
    )
    ...
    def tokenize_function(examples):
        ...
        output = tokenizer(
            text, add_special_tokens=False, max_length=max_len, truncation=True
        )  # no padding here
    ...
    # do tokenize
    prompt_raw_datasets = prompt_raw_datasets.map(
        tokenize_function, remove_columns=data_columns, num_proc=8
    )
    prompt_raw_datasets = prompt_raw_datasets.filter(
        lambda x: True if x["valid"] == 1 else False, num_proc=8
    )
...
def get_reward_data(args, tokenizer, data_files):
    raw_datasets = load_dataset(
        "json", data_files=data_files, split="train", name="reward_dataset"
    )
    ...
    def tokenize_function(examples):
        ...
        chosen_sentence = (
            chosen_sentence.replace("[END]", "")
            .replace("[BOS]", "<s>")
            .replace("[EOS]", "</s>")
        )
        reject_sentence = (
            reject_sentence.replace("[END]", "")
            .replace("[BOS]", "<s>")
            .replace("[EOS]", "</s>")
        )
        chosen_sentence = (
            chosen_sentence
            if chosen_sentence.endswith(eos_token)
            else chosen_sentence + eos_token
        )
        reject_sentence = (
            reject_sentence
            if reject_sentence.endswith(eos_token)
            else reject_sentence + eos_token
        )
        ...
        if (
            chosen_token["input_ids"].view(-1).numpy()[-1]
            in {tokenizer.eos_token_id, tokenizer.pad_token_id}
        ) and (
            reject_token["input_ids"].view(-1).numpy()[-1]
            in {tokenizer.eos_token_id, tokenizer.pad_token_id}
        ):
            ret["valid"] = 1
        else:
    ...
    raw_datasets = raw_datasets.map(
        tokenize_function, remove_columns=["prompt", "ans", "rejected"], num_proc=10
    )
    raw_datasets = raw_datasets.filter(
        lambda x: True if x["valid"] == 1 else False, num_proc=10
    )
    ...
    return PromptDataset(
        prompt_dataset, chosen_dataset, reject_dataset, tokenizer.pad_token_id, 2
    )


def get_unsupervised_data(args, tokenizer, data_files, train_phase=1, streaming=False):
    # only for chat sft
    # input datas in messages format [{role, content, lossmask}]
    data_columns = infer_dataset_columns(data_files)
    ...
    def _format_messages(messages, bos="<s>", eos="</s>"):
        masks = []
        texts = []
        for m in messages:
            masks.append(m["lossmask"])
            prefix = "[Human]\n" if m["role"] == "user" else "[AI]\n"
            texts.append(f"{bos}{prefix}{m['content']}{eos}")
            # 注意，llamaTokenizer对<s></s>周围的\n没有影响，所以句子间用不用\n没区别
        return texts, masks

    def _single_label_masking(texts, lossmasks, maxlen):
        # return inputs_ids, att_mask, labels
        input_ids_list = tokenizer(texts, add_special_tokens=False).input_ids
        labels = []
        input_ids = []
        attention_mask = []
        for iids, ismask in zip(input_ids_list, lossmasks):
            if ismask:
                labels += [-100] * len(iids)
            else:
                labels += iids
        ...
        if curr_len >= maxlen:
            ...
            input_ids = input_ids + [0] * (maxlen - curr_len)
            attention_mask = attention_mask + [0] * (maxlen - curr_len)
            labels = labels + [0] * (maxlen - curr_len)
    ...
    def tokenize_function(examples):
        ...
        if train_phase == 1:
            ...
            raise Exception("only suppert chat finetune!!!")

        texts, masks = _format_messages(messages)
        input_ids, attention_mask, labels = _single_label_masking(
            texts, masks, maxlen=max_len
        )
    ...
    if streaming:
        ...
        unsupervised_raw_datasets = unsupervised_raw_datasets.map(
            tokenize_function, remove_columns=data_columns
        )
        unsupervised_raw_datasets = unsupervised_raw_datasets.filter(
            lambda x: True if x["valid"] == 1 else False
        )
    else:
        unsupervised_raw_datasets = unsupervised_raw_datasets.map(
            tokenize_function, remove_columns=data_columns, num_proc=10
        )
        unsupervised_raw_datasets = unsupervised_raw_datasets.filter(
            lambda x: True if x["valid"] == 1 else False, num_proc=10
        )
