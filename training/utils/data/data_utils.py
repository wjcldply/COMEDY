...
def get_unsupervised_data(
    args, tokenizer=None, data_files=None, train_phase=1, streaming=False
):
    data_columns = infer_dataset_columns(data_files)
    ...
    def tokenize_function(examples):
        ...
        text = (
            text.replace("[END]", "")
            .replace("[BOS]", "<|begin_of_text|>")
            .replace("[EOS]", "<|end_of_text|>")
            .replace("<s>", "<|begin_of_text|>")
            .replace("</s>", "<|end_of_text|>")
        )
        ...
        ret["labels"] = _single_label_masking(
            ret["input_ids"].copy(), tokenizer.bos_token_id
        )
    ...
    def tokenize_function_alpaca(examples):
        ...
        text = system + text.replace("[END]", "").replace("[EOS]", "</s>").replace(
            "[BOS]", "<s>"
        ).replace("[Human]\n", "USER:\n").replace("[AI]\n", "ASSISTANT:\n").replace(
            "[HM]:", "USER:\n"
        ).replace(
        ...
        input_ids, attention_mask, labels = _single_label_masking_alpaca(
            inputs["input_ids"],
