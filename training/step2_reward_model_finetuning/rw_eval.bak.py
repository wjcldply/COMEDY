...
def load_stuff(model_name_or_path, num_padding_at_beginning):
    ...
    model = create_critic_model(
        model_name_or_path, tokenizer, None, num_padding_at_beginning, True
    )

    return model, tokenizer


def prepare_datapair(
    prompt,
    good_ans,
    bad_ans,
    tokenizer,
    ...
    chosen_sentence = (
        prompt + good_ans + end_of_conversation_token
    )  # the accept response
    reject_sentence = (
        prompt + bad_ans + end_of_conversation_token
    )  # the reject response
    ...
    batch["input_ids"] = torch.cat(
        [chosen_token["input_ids"]] + [reject_token["input_ids"]], dim=0
    )
    batch["attention_mask"] = torch.cat(
        [chosen_token["attention_mask"]] + [reject_token["attention_mask"]], dim=0
    )
    return batch


def prepare_singlesample(
    prompt,
    good_ans,
    tokenizer,
    max_seq_len=512,
...
def run_pair_comparison():
    ...
    rm_model, tokenizer = load_stuff(
        args.model_name_or_path, args.num_padding_at_beginning
    )
    rm_model.to(device)
    rm_model.eval()
