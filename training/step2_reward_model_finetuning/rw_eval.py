...
def prepare_singlbatch(
    ...
    chosen_sentence = [
        prompt + ans + end_of_conversation_token
...
if __name__ == "__main__":
    ...
    input_file = (
        "/xllm3-ft/share/general-benchmark/reward/zbench_ranking_benchmark.jsonl"
    )
    ...
    with open(input_file, "r", encoding="utf8") as f:
        for row in tqdm(f):
            ...
            prompt = [
                (
                    f"{bos}{human_pf}{item['content']}{eos}"
                    if item["role"] == "Human"
                    else f"{bos}{ai_pf}{item['content']}{eos}"
                )
