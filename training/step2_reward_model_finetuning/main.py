...
def main():
    ...
    rm_model = create_critic_model(
        args.model_name_or_path, tokenizer, ds_config, args.num_padding_at_beginning
    )
    ...
    reward_score, rejected_score, acc, loss = evaluation_reward(
        rm_model, eval_dataloader
    )
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if total_steps > 0 and total_steps % args.eval_interval == 0:
                ...
                reward_score, rejected_score, acc, dev_loss = evaluation_reward(
                    rm_model, eval_dataloader
                )
                if reward_score is not None and acc is not None:
                    print_rank_0(
                ...
                if args.global_rank == 0:
                    writer.add_scalar(
                        "Dev/chosen_last_scores", reward_score, total_steps
                    )
                    writer.add_scalar(
                        "Dev/rejected_last_scores", rejected_score, total_steps
                    )
