...
def parse_args():
    ...
    parser.add_argument(
        "--train_data_path", type=str, required=True, help="training path"
    )
    parser.add_argument(
        "--valid_data_path", type=str, required=True, help="validation path"
    )
    parser.add_argument(
        "--num_train_samples",
        type=int,
        required=True,
        help="number of rows in training datas.",
    )
    parser.add_argument(
        "--ntk_RoPE_scaling_ratio", type=int, default=1, help="scaling context window."
    )
    parser.add_argument(
        "--dtype",
        type=str,
...
def main():
    ...
    #     args.max_seq_len)
    train_dataset = get_unsupervised_data(
        args, tokenizer, args.train_data_path, train_phase=train_phase, streaming=True
    )
    eval_dataset = get_unsupervised_data(
        args, tokenizer, args.valid_data_path, train_phase=train_phase
    )
    ...
    for epoch in range(args.num_train_epochs):
        print_rank_0(
            f"Beginning of Epoch {epoch+1}/{args.num_train_epochs}", args.global_rank
        )
        model.train()
        mean_loss = 0
        for step, batch in enumerate(train_dataloader):
            ...
            if (
                total_steps > 0
                and total_steps % args.save_interval == 0
                and args.output_dir is not None
            ):
                print_rank_0(
                    f"saving model ... to step {total_steps}", args.global_rank
                )
                if args.lora_dim > 0:
                ...
                if args.zero_stage == 3:
                    ...
                    if args.global_rank == 0:
                        save_hf_format(
                            ...
                            tokenizer,
                            args,
                            sub_folder=f"checkpoint-{total_steps}",
                        )
