...
def main():
    ...
    # special_tokens_dict = {'additional_special_tokens': ["[BOS]","[EOS]","[END]"]}
    special_tokens_dict = {
        "additional_special_tokens": ["<s>", "</s>", "<unk>"]
    }  # 其实这行可有可无
    ...
    train_dataset = get_unsupervised_data(
        args, tokenizer, args.train_data_path, train_phase=train_phase
    )
    ...
    for epoch in range(args.num_train_epochs):
        ...
        if args.save_every_epoch:
            ...
            if args.zero_stage == 3:
                ...
                if args.global_rank == 0:
                    save_hf_format(
                        model, tokenizer, args, sub_folder=f"checkpoint-{total_steps}"
                    )
